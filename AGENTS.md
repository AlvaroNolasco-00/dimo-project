...
python3 -m backend.main          # Dev → localhost:8000
# Production (Koyeb):
...
Frontend API: `frontend/src/environments/` (dev: `http://localhost:8000/api`)

## Architecture

- **Backend**: FastAPI + SQLAlchemy + PostgreSQL + JWT
- **Frontend**: Angular 21, standalone components, Signals-based state
- **Multi-tenancy**: Projects (tenants) with RBAC roles: viewer < editor < manager < owner
- **Migrations**: Manual SQL in `backend/sql/` — no Alembic

## Deployment

| Component | Platform |
|----------|----------|
| Backend | Koyeb (`koyeb.yaml`) |
| Frontend | Vercel |
| Database | PostgreSQL (external) |
| GPU worker | Modal (`gpu-worker/modal_app.py`) |

## Communication

**Caveman mode always active** — terse, drop articles, fragments OK.

## ADRs

Decision records in `docs/adr/`. Template: `docs/adr/TEMPLATE.md`.
Workflow: `./docs/adr/new-adr.sh "titulo"` → fill → update `docs/adr/INDEX.md`.

## Key Guards

- `authGuard` — valid JWT
- `approvedGuard` — JWT + `is_approved=True`
- `adminGuard` — JWT + `is_admin=True`
- `projectRoleGuard(minRole)` factory — for viewer/editor/manager/owner
